#!/usr/bin/env bash
# Compile and execute every numbered Stage 2 regression sample.
# These tests validate the final compiler distribution using the canonical seed
# compiler and the runtime module store produced by the Stage 1 build.
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/../.." && pwd)"
STAGE_DIR="$PROJECT_ROOT/0.6.0-compiler"
COMPILER="$STAGE_DIR/dist/bin/clyth_compiler_bin"
STAGE1_DIST="$PROJECT_ROOT/bootstrap/0.5.1-compiler/dist"
...
main() {
    ...
    [[ -x "$STAGE0" ]] || { echo "ERROR: missing canonical Stage 0 compiler: $STAGE0" >&2; exit 1; }
