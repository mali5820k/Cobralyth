...
STAGE0_DIST="${CLYTH_STAGE0_DIST:-$PROJECT_ROOT/0.5.0-compiler-dist}"
...
cp "$STAGE0" "$DIST/libexec/clyth-stage0"
cp -R "$STORE" "$DIST/share/clyth/module-store"
mkdir -p "$DIST/share/clyth/clyth-runtime"
cp "$RUNTIME/EXTERNAL_LIBRARIES_LICENSES.md" "$DIST/share/clyth/clyth-runtime/EXTERNAL_LIBRARIES_LICENSES.md"
cp "$STAGE1_DIR/EXTERNAL_LIBRARIES_LICENSES.md" "$DIST/share/clyth/EXTERNAL_LIBRARIES_LICENSES.md"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    -h|--help)
      ...
      ;;
    -v|--version)
      printf 'Clyth bootstrap compiler 0.6.0-stage1\n'
      exit 0
      ;;
    *)
