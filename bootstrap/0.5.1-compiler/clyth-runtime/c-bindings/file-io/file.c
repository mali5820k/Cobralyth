...
static _Thread_local char* clyth_read_buffer = NULL;
static _Thread_local size_t clyth_read_capacity = 0;

static const char* clyth_prepare_read_buffer(size_t required) {
    if (required + 1 > clyth_read_capacity) {
        char* next = (char*)realloc(clyth_read_buffer, required + 1);
        if (next == NULL) return "";
        clyth_read_buffer = next;
        clyth_read_capacity = required + 1;
    }
    clyth_read_buffer[0] = '\0';
    return clyth_read_buffer;
}

int32_t clyth_file_replace_line(const char* path, int64_t line_index, const char* content) {
    if (path == NULL || content == NULL || line_index < 0) return -1;
    ...
    if (temporary_path == NULL) { fclose(input); return -1; }
    snprintf(temporary_path, template_length, "%s.XXXXXX", path);

    int temporary_fd = mkstemp(temporary_path);
    if (temporary_fd < 0) { free(temporary_path); fclose(input); return -1; }
    FILE* output = fdopen(temporary_fd, "wb");
    if (output == NULL) { close(temporary_fd); unlink(temporary_path); free(temporary_path); fclose(input); return -1; }
    ...
    free(line);
    int failed = ferror(input) || fflush(output) != 0;
    if (fclose(input) != 0) failed = 1;
    if (fclose(output) != 0) failed = 1;

    if (!replaced || failed || rename(temporary_path, path) != 0) {
...
const char* clyth_file_read(const char* path, int64_t offset, int64_t max_bytes) {
    if (path == NULL || offset < 0 || max_bytes < 0) return "";
    if (max_bytes == 0) return clyth_prepare_read_buffer(0);

    uv_fs_t open_req;
    int open_result = uv_fs_open(file_loop(), &open_req, path, O_RDONLY, 0, NULL);
    uv_fs_req_cleanup(&open_req);
    if (open_result < 0) return "";

    clyth_prepare_read_buffer((size_t)max_bytes);
    if (clyth_read_buffer == NULL) {
        clyth_uv_close_file((uv_file)open_result);
        return "";
    }

    uv_buf_t buffer = uv_buf_init(clyth_read_buffer, (unsigned int)max_bytes);
    uv_fs_t read_req;
    int result = uv_fs_read(file_loop(), &read_req, (uv_file)open_result, &buffer, 1, offset, NULL);
    uv_fs_req_cleanup(&read_req);
    clyth_uv_close_file((uv_file)open_result);
    if (result < 0) {
        clyth_read_buffer[0] = '\0';
    ...
    clyth_read_buffer[result] = '\0';
    return clyth_read_buffer;
}

const char* clyth_file_read_line(const char* path, int64_t line_index) {
    if (path == NULL || line_index < 0) return "";
    ...
    size_t length = 0;
    size_t capacity = 256;
    if (clyth_prepare_read_buffer(capacity) == NULL) {
        clyth_uv_close_file((uv_file)open_result);
        return "";
    }
    ...
    while (!found) {
        ...
        for (int i = 0; i < result; ++i) {
            ...
            if (current_line == line_index) {
                ...
                if (length + 2 > clyth_read_capacity) {
                    capacity = clyth_read_capacity * 2;
                    if (clyth_prepare_read_buffer(capacity) == NULL) {
                        clyth_uv_close_file((uv_file)open_result);
                        return "";
                    }
                }
                clyth_read_buffer[length++] = character;
            } else if (character == '\n') {
                current_line += 1;
            }
        }
...
int32_t replace_line(const char* path, int64_t line_index, const char* content) { return clyth_file_replace_line(path, line_index, content); }
...
const char* file_read(const char* path, int64_t offset, int64_t max_bytes) { return clyth_file_read(path, offset, max_bytes); }
const char* file_read_line(const char* path, int64_t line_index) { return clyth_file_read_line(path, line_index); }
