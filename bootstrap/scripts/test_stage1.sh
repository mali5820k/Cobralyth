#!/usr/bin/env bash
# Compile and execute every numbered Stage 1 regression sample.
# Per-test logs remain in a generated regression directory until cleanup so a
# failed bootstrap can be diagnosed without contaminating the source tree.
set -Eeuo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
STAGE_DIR="$(cd "$SCRIPT_DIR/../0.5.1-compiler" && pwd)"
