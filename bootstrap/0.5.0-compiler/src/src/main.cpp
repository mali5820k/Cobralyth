...
static std::filesystem::path resolve_include_path(
    ...
    if (!runtime_root.empty()) {
        ...
        // A local file import such as `import dma/dma.clyth` resolves to
...
static bool has_clyth_suffix(const std::string& target) {
    constexpr std::string_view suffix = ".clyth";
    return target.size() >= suffix.size()
        && target.compare(target.size() - suffix.size(), suffix.size(), suffix) == 0;
}

static std::string module_name_from_import_target(const std::string& target) {
    const std::size_t slash = target.find_last_of("/\\");
    const std::size_t dot = target.find_last_of('.');
    const std::size_t split = std::max(
        slash == std::string::npos ? 0 : slash + 1,
        dot == std::string::npos ? 0 : dot + 1
    );
    return target.substr(split);
}

static std::string default_import_alias(const std::string& target) {
    std::filesystem::path path(target);
    if (has_clyth_suffix(target)) {
        return path.stem().string();
    }
    return module_name_from_import_target(target);
}

static std::string rewrite_namespace_qualifiers(
    ...
    for (const auto& alias : aliases) {
        ...
        const std::regex qualifier("\\b" + alias + "\\s*\\.");
        out = std::regex_replace(out, qualifier, "");
    }
    return out;
}

static bool is_runtime_package_include_name(const std::string& include_target) {
    return !include_target.empty() && !has_clyth_suffix(include_target);
}
...
static bool read_clyth_source_with_includes_impl(
    ...
    if (active_stack.count(normalized) > 0) {
        fmt::print(stderr, "ERROR: import cycle detected at {}\n", normalized.string());
        return false;
    }
    ...
    const std::regex legacy_include_pattern(R"(^\s*include\s+([^;\s]+)\s*;?\s*$)");
    std::istringstream input(scanner.to_string());
    std::string line;
    std::vector<std::string> namespace_aliases;

    output << "// begin import unit: " << normalized.string() << "\n";

    while (std::getline(input, line)) {
        std::smatch match;
        bool legacy_include = false;
        if (!std::regex_match(line, match, import_pattern)) {
            legacy_include = std::regex_match(line, match, legacy_include_pattern);
        }
        if (!match.empty()) {
            std::string import_target = strip_quotes(match[1].str());
            std::string alias;
            if (!legacy_include && match.size() > 2 && match[2].matched) {
                alias = match[2].str();
            } else {
                alias = default_import_alias(import_target);
            }
            if (!alias.empty()) {
                namespace_aliases.push_back(alias);
            }

            if (legacy_include) {
                fmt::print(stderr,
                    "WARN: '{}' uses legacy 'include'; replace it with unquoted 'import {}'.\n",
                    normalized.string(), import_target);
            }

            if (has_clyth_suffix(import_target)) {
                const std::filesystem::path import_path = resolve_include_path(
                    base_dir, runtime_root, std::filesystem::path(import_target));
                if (!read_clyth_source_with_includes_impl(
                        import_path, runtime_root, active_stack, already_included, included_paths, output)) {
                    return false;
                }
                continue;
            }

            const std::string module_name = module_name_from_import_target(import_target);
            const bool runtime_package_exists = !runtime_root.empty()
                && std::filesystem::exists(runtime_root / "modules" / ("module-" + module_name) / "module.json");
            if (is_runtime_package_include_name(import_target) && runtime_package_exists) {
                std::set<std::string> active_modules;
                std::set<std::string> resolved_modules;
                std::vector<std::filesystem::path> module_sources;
                if (!collect_runtime_module_sources(
                        runtime_root, module_name, active_modules, resolved_modules, module_sources)) {
                    return false;
                }
                for (const auto& module_source : module_sources) {
                    if (!read_clyth_source_with_includes_impl(
                            module_source, runtime_root, active_stack, already_included, included_paths, output)) {
                        return false;
                    }
                }
                continue;
            ...
            fmt::print(stderr,
                "ERROR: Module import '{}' was not found in '{}'. Local files must end in .clyth.\n",
                import_target, runtime_root.string());
            return false;
        }

        output << rewrite_namespace_qualifiers(line, namespace_aliases) << "\n";
    }

    output << "// end import unit: " << normalized.string() << "\n";
...
static int parse_clyth_file(CompilerOptions& opts) {
    ...
    // Runtime packages are loaded only through explicit import declarations such as
    // `import collections`. The compiler no longer injects List/Set/Map sources
