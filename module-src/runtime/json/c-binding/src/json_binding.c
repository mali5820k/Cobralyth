...
/*
 * yyjson is fetched at build time into Clyth's ignored vendor cache.
...
char* json_object_get_string(const char* text, const char* key) {
    yyjson_doc* doc = clyth_json_read_doc(text);
    if (!doc) return strdup("");
    yyjson_val* root = yyjson_doc_get_root(doc);
    yyjson_val* value = (root && yyjson_is_obj(root) && key) ? yyjson_obj_get(root, key) : NULL;
    const char* raw = (value && yyjson_is_str(value)) ? yyjson_get_str(value) : NULL;
