...
typedef struct clyth_epoll_server {
    int listen_fd;
    int epoll_fd;
    int32_t port;
    int64_t router_handle;
...
typedef struct clyth_client {
    int fd;
    clyth_epoll_server* server;
    char* request;
...
static int64_t register_response(clyth_client* client) {
...
static int write_all_socket(int fd, const char* data, size_t length) {
    size_t sent = 0;
    while (sent < length) {
...
static void write_response(clyth_client* client, int status_code, const char* content_type, const char* body) {
    if (client == NULL || client->response_started) return;
    client->response_started = 1;
    const char* reason = status_code == 200 ? "OK" : status_code == 400 ? "Bad Request" : status_code == 404 ? "Not Found" : status_code == 413 ? "Payload Too Large" : "Error";
    if (body == NULL) body = "";
    if (content_type == NULL) content_type = content_type_for_payload(body);
    ...
    if (write_all_socket(client->fd, header, (size_t)header_length) == 0 && body_length != 0)
        (void)write_all_socket(client->fd, body, body_length);
}
...
static void maybe_handle_request(clyth_client* client) {
...
static void close_client(clyth_client* client) {
    if (!client) return;
    if (client->server && client->server->epoll_fd >= 0 && client->fd >= 0)
        epoll_ctl(client->server->epoll_fd, EPOLL_CTL_DEL, client->fd, NULL);
    if (client->fd >= 0) close(client->fd);
    free(client->request);
    free(client);
}

static int accept_clients(clyth_epoll_server* server) {
    for (;;) {
        ...
        clyth_client* client = (clyth_client*)calloc(1, sizeof(clyth_client));
        if (!client) { close(fd); return -ENOMEM; }
        client->fd = fd; client->server = server;
        struct epoll_event ev; memset(&ev, 0, sizeof(ev));
        ev.events = EPOLLIN | EPOLLRDHUP; ev.data.ptr = client;
        if (epoll_ctl(server->epoll_fd, EPOLL_CTL_ADD, fd, &ev) != 0) { close_client(client); continue; }
    }
}

static int read_client(clyth_client* client) {
    char buffer[16 * 1024];
    for (;;) {
        ...
        if (n == 0) return 1;
        size_t required = client->request_length + (size_t)n + 1;
        if (required > CLYTH_MAX_REQUEST_BYTES) { write_response(client, 413, "text/plain; charset=utf-8", "Request too large"); return 1; }
        if (required > client->request_capacity) {
            size_t cap = client->request_capacity ? client->request_capacity * 2 : 4096;
            while (cap < required) cap *= 2;
            char* next = realloc(client->request, cap);
            if (!next) return -ENOMEM;
            client->request = next; client->request_capacity = cap;
        }
        memcpy(client->request + client->request_length, buffer, (size_t)n);
        client->request_length += (size_t)n; client->request[client->request_length] = '\0';
        maybe_handle_request(client);
        if (client->response_started) return 1;
    }
}

static int32_t clyth_http_serve_epoll(int32_t port, int64_t router_handle) {
    if (port <= 0 || port > 65535 || !router_exists(router_handle)) return -EINVAL;
    clyth_epoll_server server; memset(&server, 0, sizeof(server));
    server.listen_fd = -1; server.epoll_fd = -1; server.port = port; server.router_handle = router_handle;
    server.listen_fd = socket(AF_INET, SOCK_STREAM, 0);
    if (server.listen_fd < 0) return -errno;
    int yes = 1; (void)setsockopt(server.listen_fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes));
    if (set_nonblocking(server.listen_fd) != 0) { int e=-errno; close(server.listen_fd); return e; }
    struct sockaddr_in address; memset(&address, 0, sizeof(address));
    address.sin_family = AF_INET; address.sin_addr.s_addr = htonl(INADDR_ANY); address.sin_port = htons((uint16_t)port);
    if (bind(server.listen_fd, (struct sockaddr*)&address, sizeof(address)) != 0) { int e=-errno; close(server.listen_fd); return e; }
    if (listen(server.listen_fd, 128) != 0) { int e=-errno; close(server.listen_fd); return e; }
    server.epoll_fd = epoll_create1(EPOLL_CLOEXEC);
    if (server.epoll_fd < 0) { int e=-errno; close(server.listen_fd); return e; }
    struct epoll_event listen_event; memset(&listen_event,0,sizeof(listen_event));
    listen_event.events = EPOLLIN; listen_event.data.ptr = &server;
    if (epoll_ctl(server.epoll_fd, EPOLL_CTL_ADD, server.listen_fd, &listen_event) != 0) { int e=-errno; close(server.epoll_fd); close(server.listen_fd); return e; }
    struct epoll_event events[128];
    for (;;) {
        int count = epoll_wait(server.epoll_fd, events, 128, -1);
        if (count < 0) { if (errno == EINTR) continue; int e=-errno; close(server.epoll_fd); close(server.listen_fd); return e; }
        for (int i=0;i<count;i++) {
            if (events[i].data.ptr == &server) { (void)accept_clients(&server); continue; }
            clyth_client* client = (clyth_client*)events[i].data.ptr;
            if (!client) continue;
            int done = 0;
            if (events[i].events & (EPOLLERR|EPOLLHUP|EPOLLRDHUP)) done = 1;
            else if (events[i].events & EPOLLIN) done = read_client(client) != 0;
            if (done) close_client(client);
        }
    }
}
...
int32_t clyth_server_start(int64_t server_handle) {
    ...
    return clyth_http_serve_epoll(configuration->port, configuration->router_handle);
}
