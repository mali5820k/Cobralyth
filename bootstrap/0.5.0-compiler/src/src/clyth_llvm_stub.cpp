...
bool ClythLLVMCodegen::emit_function_body(const ast::NodePtr& node, const semantic::SemanticResult& semantics) {
    ...
    for (llvm::Argument& arg : function->args()) {
        ...
        if (modern_main && index == 1 && !params.empty()) {
            ...
            // Native process argv is an opaque char** paired with the hidden
            // __clyth_argc value, not a Clyth dynamic-array struct. Registering
            // it as a normal string[] backing makes args.length load arbitrary
            // words from argv and eventually walk into the environment block.
        } else if (!modern_main && index < params.size()) {
...
llvm::Value* ClythLLVMCodegen::emit_postfix(const ast::NodePtr& node, const semantic::SemanticResult& semantics) {
    ...
    for (const auto& child : node->children) {
        ...
        if (child->kind == ast::NodeKind::MemberAccessExpr) {
            ...
            if (member == "length") {
                if (*maybe_base_name == "args") {
                    llvm::AllocaInst* argc_alloca = lookup_local("__clyth_argc");
                    if (argc_alloca != nullptr) {
                        llvm::Value* argc_value = builder.CreateLoad(
                            argc_alloca->getAllocatedType(), argc_alloca, "args.length.argc"
                        );
                        current_value = builder.CreateIntCast(
                            argc_value, llvm::Type::getInt64Ty(context), false, "args.length"
                        );
                        consumed_suffix = true;
                        continue;
                    }
